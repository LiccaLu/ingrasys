...
if page == "01  Upload":
    ...
    with col1:
        ...
        st.markdown('<div class="section-title">Attendance file</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="panel">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">Leave file</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        """
        <div class="rule-box">
        <b>Attendance rule:</b><br>
        Scheduled time exists + both actual clock-in and clock-out are blank → <b>Absent</b>.<br>
        When the same employee has an AL or Other Leave period overlapping that shift,
        the final result becomes <b>Leave Approved</b>.
        </div>
        """,
        unsafe_allow_html=True,
    )
        ...
        try:
            with st.spinner("Processing Attendance, AL and Other Leave..."):
                ...
                history_item = {
                    ...
                    "Leave Approved": int((statuses == "Leave Approved").sum()),
                    "Forgot Clock-in": int((statuses == "Forgot Clock-in").sum()),
                    "Forgot Clock-out": int((statuses == "Forgot Clock-out").sum()),
            ...
            with c1:
                metric_card("Attendance rows", f"{len(attendance):,}", f"Sheet: {sheet}")
            with c2:
                metric_card("Absent", f"{(statuses == 'Absent').sum():,}", "After leave matching")
            with c3:
                metric_card("Leave Approved", f"{(statuses == 'Leave Approved').sum():,}", "AL + Other Leave")
            with c4:
                missing_punch = statuses.isin(["Forgot Clock-in", "Forgot Clock-out"]).sum()
                metric_card("Forgot Punch", f"{missing_punch:,}", "Incomplete punch records")
