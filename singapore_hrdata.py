...
- Absence percentage
    ...
    with st.container(border=True):
        ...
        if (
            ...
            # ============================================================
            # Exclude selected employees from Daily Absence Rate
            # ============================================================
            EXCLUDED_DAILY_ABSENCE_NAMES = {
                "AHMAD AZFAR BIN ABDUL HAMID",
                "ALMANDA MIK",
                "AMANDA DING",
                "AMELIA CHAN JOO YI",
                "ANG FANG LIM",
                "BALWINDER SINGH SO JAGDISH SINGH",
                "BOBBY ONG WAI HOONG",
                "CHAN ZHI JING",
                "CHANG SHU-WEI",
                "CHE CHEE YEONG",
                ...
                "JAMES CHOW SINAN",
                "JIAN YUXING",
                "JOSHUA HOR SHEN YANG",
                "JOY TIE YEE YUNG",
                "KAUNG HTET LU",
                "KAYDEN LUA XIN XUN",
                "KAYYANICAL SURESH SUDIN",
                "KHAIRUN NABILAH BINTE AHMAD AKHIA",
                "KHER MENG JIONG",
                "KHIN MAY OO",
                "KONG CHEE KEAT",
                "LAI SEOK TONG",
                ...
                "LOW QING HUI",
                "LUA CHONG SHEN",
                "MASHIDAYU BINTE MOHAMED MISRAN",
                "MUHAMMAD AFIQ BIN MOHAMED SHAFEI",
                ...
                "NGEOW BIH MAN",
                "NI XIN",
                "NUR SABRINA BINTE AZMAN",
                "ONG KANG SENG",
                "PANG HEE LIM",
                "QIN WEIWEI",
                "QIN WEIWEI",
                "SALLY NG SEE YING",
                "SARAVANAN S/O SUPPAYIAH",
                "SHEN YU CHUAN (SHEN YUJUAN)",
                "SOH XIN KAI",
                "SOON YING ZE",
                "SYAHRUL HAFIZI BIN MOHAMED",
                "TAI PEI TING",
                "TAN CHEE YONG",
                "TAN JIA HUA",
                "ENG WEI CHIANG",
                ...
                "TAY WEU CHENG",
                "TEE CHOR HENG",
                "TENGKU MUHAMMAD SHAH AZRI",
                "TEO AH SIEW",
                "TEO JIA XIN",
                "TEO LIM KOON (ZHANG LINQIN)",
                "TEO SIEW HWA",
                "TEO WEI TECK",
                "TEO ZHI JIE",
                "TEY SHEAW REEI",
            ...
            if name_col in daily_scheduled.columns:
            
                daily_scheduled["_name_clean"] = (
                    daily_scheduled[name_col]
                    .fillna("")
                    .astype(str)
                    .str.replace("\u00a0", " ", regex=False)
                    .str.strip()
                ...
                excluded_names_clean = {
                    str(name)
                    .replace("\u00a0", " ")
                    .strip()
                    .upper()
                    for name in EXCLUDED_DAILY_ABSENCE_NAMES
                    if str(name).strip()
                }
            
                daily_scheduled = daily_scheduled[
                    ~daily_scheduled["_name_clean"].isin(
                        excluded_names_clean
                    )
                ].copy()
            
                daily_scheduled = daily_scheduled.drop(
                    columns=["_name_clean"]
                )
